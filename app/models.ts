...
export type Subscription = {
  ...
  startDate?: string; nextDate: string; category: string; status: Status; color: string; iconUrl?: string; reminderOffsets?: number[]; cadenceInterval?: number; anchorDay?: number;
};

export type Bill = { id: string; baseAmount?: number; baseCurrency?: string; exchangeRateDate?: string; referenceRates?: Record<string,number>; source?: 'schedule' | 'manual' | 'renewal'; subscriptionId?: string; subscription: string; date: string; amount: number; currency: string; status: BillStatus };
