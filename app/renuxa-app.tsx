...
import { billingDates, cadenceLabel, cadenceUnit, localDateKey, subscriptionPlan, localSubscriptionBills } from './billing';
...
export default function RenuxaApp() {
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    Promise.all([
      apiRequest<Record<string,unknown>[]>('/subscriptions', {}, token), allBills(token), apiRequest<Record<string,unknown>[]>('/notifications', {}, token),
    ]).then(([remoteSubs, remoteBills, remoteNotices]) => {
      if (cancelled) return;
      setLoadedToken(token);
      setRefreshError('');
      setSubscriptions((remoteSubs as Record<string, unknown>[]).map(remoteSubscription));
      setBills((remoteBills as Record<string, unknown>[]).map((row) => ({ id:String(row.id), baseAmount:row.base_amount==null?undefined:Number(row.base_amount), baseCurrency:row.base_currency?String(row.base_currency):undefined, exchangeRateDate:row.exchange_rate_date?String(row.exchange_rate_date):undefined, referenceRates:Object.fromEntries(Object.entries((row.reference_rates??{}) as Record<string,unknown>).map(([code,rate])=>[code,Number(rate)])), subscriptionId:String(row.subscription_id), subscription:String(row.subscription_name), date:String(row.due_date), amount:Number(row.amount), currency:String(row.currency), status:String(row.status) as BillStatus })));
  ...
  const addSubscription = async (sub: Subscription) => {
    const saved = token ? remoteSubscription(await apiRequest('/subscriptions', { method:'POST', body:JSON.stringify({ name:sub.name, plan_name:sub.plan, amount:String(sub.amount), currency:sub.currency, cadence_unit:cadenceUnit(sub.cadence), cadence_interval:sub.cadenceInterval??1, start_date:sub.startDate, next_billing_date:sub.startDate?undefined:sub.nextDate, category:sub.category, icon_url:sub.iconUrl, reminder_offsets:sub.reminderOffsets ?? [7,3,1] }) }, token)) : sub;
    if (!apiUrl) setBills((current)=>localSubscriptionBills(saved,current));
  ...
  const editSubscription = async (sub: Subscription) => {
    const saved = token ? remoteSubscription(await apiRequest(`/subscriptions/${sub.id}`, { method:'PATCH', body:JSON.stringify({ name:sub.name, plan_name:sub.plan, amount:String(sub.amount), currency:sub.currency, cadence_unit:cadenceUnit(sub.cadence), cadence_interval:sub.cadenceInterval??1, start_date:sub.startDate, next_billing_date:sub.startDate?undefined:sub.nextDate, category:sub.category, icon_url:sub.iconUrl, reminder_offsets:sub.reminderOffsets ?? [7,3,1] }) }, token)) : sub;
    if (!apiUrl) setBills((current)=>localSubscriptionBills(saved,current));
...
function SubscriptionModal({ locale, initial, onClose, onSave }: { locale:Locale; initial:Subscription|null; onClose:()=>void; onSave:(sub:Subscription)=>Promise<void> }) {
  ...
  const [name,setName]=useState(initial?.name??''); const [plan,setPlan]=useState(initial?.plan??''); const [amount,setAmount]=useState(initial?String(initial.amount):''); const [currency,setCurrency]=useState(initial?.currency??'CNY'); const [cadence,setCadence]=useState(cadenceUnit(initial?.cadence??'month')); const [cadenceInterval,setCadenceInterval]=useState(String(initial?.cadenceInterval??1)); const [startDate,setStartDate]=useState(initial?.startDate??(initial?'':localDateKey(new Date()))); const [category,setCategory]=useState(initial?.category??'工作效率'); const [iconUrl,setIconUrl]=useState(initial?.iconUrl??''); const [iconBusy,setIconBusy]=useState(false); const [reminders,setReminders]=useState<number[]>(()=>initial?.reminderOffsets??(()=>{try{return JSON.parse(localStorage.getItem('renuxa.reminders')??'[7,3,1]')}catch{return [7,3,1]}})());
  ...
  const submit=async(event:FormEvent)=>{
    ...
    setBusy(true);setError('');
    try {
      const planDates = subscriptionPlan({startDate,nextDate:startDate,cadence,cadenceInterval:interval});
      await onSave({id:initial?.id??createLocalId(),name:name.trim(),plan:plan.trim()||'标准方案',amount:numeric,currency,cadence,startDate,nextDate:planDates.nextDate,category,status:cadence==='once'&&planDates.paidDates.length?'cancelled':initial?.status??'active',color:initial?.color??colors[name.length%colors.length],iconUrl:iconUrl||undefined,reminderOffsets:reminders,cadenceInterval:cadence==='once'?1:interval,anchorDay:new Date(`${startDate}T00:00:00`).getDate()});
    } catch (reason) {
